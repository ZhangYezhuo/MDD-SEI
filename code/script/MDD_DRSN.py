...
def main(args:argparse.Namespace):
    ...
    # test data (ALSO FOR UNSUPERVISED TRAINING)
    if args.inner_train:
        if "bes" in args.class_names:
    ...
    train_target_iter = ForeverDataIterator(DataLoader(ConcatDataset([test_dataset for (test_dataset, *_) in test_pack]), batch_size=args.batch_size, shuffle=True, num_workers=args.workers, drop_last=True))
    ...
    classifier = ImageClassifier(classifier.backbone, len(args.device_train), bottleneck_dim=args.bottleneck_dim, width=args.bottleneck_dim, pool_layer=pool_layer).to(device)
    ...
    lr_scheduler = LambdaLR(optimizer, lr_lambda=lambda epoch: (epoch%10+1)*0.1)
    # lr_scheduler_warm = LinearLR(optimizer_warm, start_factor=1/args.warm_epoch, end_factor=1, total_iters=args.warm_epoch)
    ...
    for epoch in tqdm(range(args.warm_epoch), desc="WARMING UP  "):
        ...
        train(train_source_iter, train_target_iter, classifier, mdd, optimizer_warm,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
          classifier: ImageClassifier, mdd: MarginDisparityDiscrepancy, optimizer,
          lr_scheduler, epoch: int, args: argparse.Namespace, phase = "train"):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='MDD_DRSN')
    # csv logger
    parser.add_argument("--era_name", type=str, default="MDD_DRSN")    
    ...
    parser.add_argument('--no-pool', action='store_true',
                        help='no pool layer after the feature extractor.', default=False)
